...
router = APIRouter(tags=["base-v7-inference"])
    ...
    async def process_latest_frames() -> None:
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                logger.exception("Base v7 inference failed for session %s frame %s", session_id, frame.meta.frame_id)
                stop_event.set()
                await send_error("inference_failed", "Base v7 WebSocket inference failed. Check backend model/runtime logs.", frame.meta.frame_id)
