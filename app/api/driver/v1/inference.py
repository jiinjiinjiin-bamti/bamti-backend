...
router = APIRouter(tags=["driver4-v1-inference"])
...
async def infer_frame(
    ...
    if not frame_bytes:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Frame file must not be empty.")
    ...
    result = await get_runner("driver4-torch").infer(frame_bytes)
