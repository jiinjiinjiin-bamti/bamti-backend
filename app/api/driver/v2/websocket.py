...
router = APIRouter(tags=["driver4-v2-inference"])
    ...
    try:
        while True:
            ...
            if message_type == "session_end":
                ...
                await websocket.send_json({"type": "session_ended", "sessionId": session_end.session_id, "serverTime": _server_time_ms()})
            ...
            if len(frame_bytes) > settings.max_frame_bytes:
                await _send_error(websocket, "frame_too_large", f"Frame exceeds max_frame_bytes={settings.max_frame_bytes}.", frame_meta.frame_id)
                continue
