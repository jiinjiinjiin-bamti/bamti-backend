...
from app.api.v2.schemas import SessionEndMessage, SessionStartMessage
from app.core.config import settings
from app.inference.manifest import get_runner


router = APIRouter(tags=["driver4-v4-inference"])
...
class FrameMetaMessage(BaseModel):
    ...
    content_type: Literal["application/x-rgb24", "image/jpeg"] = Field(alias="contentType")
...
async def inference_stream(websocket: WebSocket) -> None:
    ...
    async def process_latest_frames() -> None:
        ...
        while not stop_event.is_set():
            ...
            try:
                if frame.meta.content_type == "application/x-rgb24":
                    result = await runner.infer_raw_rgb(frame.frame_bytes, frame.meta.width, frame.meta.height)
                else:
                    result = await runner.infer(frame.frame_bytes)
            except Exception:
                logger.exception("Driver4 raw RGB inference failed for session %s frame %s", session_id, frame.meta.frame_id)
                stop_event.set()
                await send_error("inference_failed", "Driver4 v4 inference failed. Check backend model/runtime logs.", frame.meta.frame_id)
        ...
        while True:
            ...
            async with queue_lock:
                if pending_frame is not None:
                    ...
                    dropped_frames += 1
                pending_frame = QueuedFrame(frame_meta, frame_bytes, server_received_at)
                frame_available.set()
