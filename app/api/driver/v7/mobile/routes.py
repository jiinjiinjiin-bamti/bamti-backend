...
router = APIRouter(prefix="/mobile", tags=["driver4-v7-mobile"])
    ...
    try:
        while True:
            ...
            try:
                ...
                logger.exception("Driver4 mobile v7 inference failed for session %s frame %s", session_id, frame_meta.frame_id)
...
@router.websocket("/sessions/{session_id}/signaling/{peer}")
async def mobile_signaling(websocket: WebSocket, session_id: str, peer: str) -> None:
    from app.api.aihub.mobile_v6.routes import mobile_signaling as aihub_mobile_signaling

    await aihub_mobile_signaling(websocket, session_id, peer)
