...
router = APIRouter(tags=["driver4-v7-inference"])
    ...
    async def process_latest_frames() -> None:
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                logger.exception("Driver4 v7 inference failed for session %s frame %s", session_id, frame.meta.frame_id)
                stop_event.set()
                await send_error("inference_failed", "Driver4 v7 WebSocket inference failed. Check backend model/runtime logs.", frame.meta.frame_id)
        ...
        while True:
            ...
            async with queue_lock:
                if pending_frame is not None:
                    dropped_frames += 1
                pending_frame = QueuedFrame(meta=frame_meta, frame_bytes=frame_bytes, server_received_at=server_received_at)
                frame_available.set()
