...
def get_runner(name: str = "bamti-torch") -> InferenceRunner:
    ...
    if name in {"base-torch", "torch-base"}:
        from app.inference.torch_runner import BamtiTorchRunner

        return BamtiTorchRunner(model_path=settings.base_model_path)
    if name in {"base-torch-compiled", "torch-base-compiled"}:
        from app.inference.torch_runner import BamtiTorchRunner

        return BamtiTorchRunner(use_compiled_model=True, model_path=settings.base_model_path)
