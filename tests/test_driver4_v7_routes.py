...
class FakeDriver4RawRunner(FakeDriver4Runner):
...
def test_driver4_v7_websocket_returns_temporal_risk_scores(monkeypatch) -> None:
    ...
    with client.websocket_connect("/api/driver/v7/inference/stream") as websocket:
        ...
        assert result["riskScoring"]["activationThreshold"] == 0.4
        assert result["riskScoring"]["chargeRate"] == 60.0
...
def test_driver4_v7_rawrgb_websocket_returns_lightweight_temporal_risk_scores(monkeypatch) -> None:
    requested_runner_names: list[str] = []

    def fake_get_runner(name: str):
        requested_runner_names.append(name)
        return FakeDriver4RawRunner()

    monkeypatch.setattr("app.api.v3.websocket.get_runner", fake_get_runner)
    client = TestClient(app)

    with client.websocket_connect("/api/driver/v7-rawrgb/inference/stream") as websocket:
        websocket.send_json(
            {
                "type": "session_start",
                "sessionId": "session-1",
...
def test_driver4_v7_websocket_uses_frame_time_for_risk_scores(monkeypatch) -> None:
    ...
    with client.websocket_connect("/api/driver/v7/inference/stream") as websocket:
        ...
        assert result["riskScores"]["phone_operation"] == 24.0
